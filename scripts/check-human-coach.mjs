...
try{
 for(const width of [390,1280]){
  ...
  await page.route('**/api/**',async route=>{
   ...
   else if(pathname==='/api/auth/login'){authenticated=true;data={authenticated:true};}
  ...
  role='coach';await page.reload();await page.getByRole('button',{name:'Открыть кабинет',exact:true}).click();await page.getByLabel('Следующий шаг',{exact:true}).selectOption('complete');await page.getByLabel('Объясни решение и следующий шаг').fill('Решение объяснено. Проверим такой же сигнал в другой ситуации.');await page.getByRole('button',{name:'Сохранить обратную связь',exact:true}).click();await page.getByText('История тренировок',{exact:true}).waitFor();assert.equal(tasks[0].state,'completed');
  ...
  revoked=false;await page.locator('#logout').click();await page.locator('#auth').waitFor({state:'visible'});await page.goto(origin+'/human-coach#coach_invite='+'T'.repeat(43));await page.locator('#email').fill('student@example.test');await page.locator('#password').fill('Synthetic passphrase 2026');await page.locator('#auth-submit').click();await page.getByLabel('Как тренеру к тебе обращаться?').waitFor();assert.equal(new URL(page.url()).hash,'');await page.getByLabel('Как тренеру к тебе обращаться?').fill('Никита');await page.getByRole('button',{name:'Принять приглашение',exact:true}).click();await page.locator('.human-intro h2').waitFor();
