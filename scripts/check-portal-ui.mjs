...
const report={
  ...
  coaching:{status:'ready',summary:'Начни с эпизода на линии.',points:[{title:malicious,observation:'Наблюдение из реплея.',advice:'Совет для проверки.',evidence_ids:['death-1']}]},
  coverage:{complete:true,limits:['Причины решений и видимость не угадываются.']}
};
try {
  for(const width of [390,1440]) {
    const page=await browser.newPage({viewport:{width,height:1000}});
    let authenticated=false, bound=false, job=null, uploaded=false;
    ...
    await page.route('**/api/**',async route=>{
      ...
      else if(job&&endpoint===`/api/replays/${job.id}`) body={replay:job,parts:uploaded?[1]:[],report:job.state==='ready'?{...report,...(width===1440?{coaching:{status:'unavailable',summary:'',points:[]}}:{})}:null};
  ...
  console.log('Portal .dem upload→report, selected-player binding, shared gold/XP timeline, evidence links, safe text, account navigation, mobile layout and WCAG passed (mocked API; no paid calls).');
