...
export async function POST(request:Request,context:Context){try{
  ...
  if(row.state==="ready") {
    if(row.payload!==payload)throw new AccountApiError("Результат уже сохранён с другими данными.",409);
    return accountJson({ready:true,matchId:match.matchId});
  }
  const result=await db.prepare("UPDATE replay_uploads SET state='ready',match_id=?1,normalized_payload=?2,completion_token=?4,lease_token=NULL,lease_expires_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?3 AND state='processing' AND lease_token=?4 AND lease_expires_at>CURRENT_TIMESTAMP").bind(match.matchId,payload,row.id,token).run();
