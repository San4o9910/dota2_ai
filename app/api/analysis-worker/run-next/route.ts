...
export async function POST(request:Request){try{
  ...
  const work=(async()=>{
    for(const job of candidates.results){
      ...
      const result=await runOwnedAnalysis(job.userId,job.id,{store,cache:new D1ScanMatchCache(db),apiKey:runtime.apiKey!,model:runtime.model!,allowedModels:runtime.allowedModels,signal});
