...
export const STAGES: Array<{
...
}> = [
  {
    ...
    to: 0,
    focusTime: 0,
    verdict: "Dire проще начинать общую драку и превращать победу в строения.",
    explanation: "Centaur, Black Hole, Requiem и Eclipse дают Dire надёжный 5×5, а SF, Luna и Enigma быстро бьют здания. Radiant важнее искать раздельные цели и не стоять компактно.",
    evidence: ["10 пиков подтверждены матчем", "Роли и линии — оценка OpenDota", "Персональный план — после выбора героя"],
  },
  {
    key: "laning",
    label: "Лайнинг",
    ...
    to: 720,
    focusTime: 461,
    verdict: "Равный счёт скрывал проигрыш по крипам: 92 против 190 к десятой минуте.",
    explanation: "Главный разрыв возник в центре: Shadow Fiend имел 63/8 LH/DN и 4 735 золота, Windranger — 18/4 и 2 276. Ротация Radiant на 7:01 дала два убийства, но оставила SF центральную T1 на 7:41.",
  ...
  },
  {
    ...
    to: 1800,
    focusTime: 1550,
    verdict: "Dire связал Blink-тайминги с двумя выигранными драками и Tormentor.",
    explanation: "Enigma получил Blink на 14:31 и провёл Black Hole в окне 15:24; следующая драка 16:40 закончилась 3:0. К 26:00 перевес Dire достиг 13 668 золота, а на 25:50 команда забрала Tormentor.",
  ...
  },
  {
    ...
    to: 2829,
    focusTime: 2609,
    verdict: "Roshan был правильным. Матч сломала драка 3×4 после потери Aegis.",
    explanation: "Radiant честно вернулся: на 36:00 имел +4 143 XP, а на 38:46 забрал Roshan в ситуации 5×3. После снятия Aegis на 42:30 команда не отошла: на 43:06 Juggernaut и Ogre были мертвы, а оставшаяся тройка приняла 3×4 и погибла 0–3.",
    evidence: ["Aegis Juggernaut: 38:46 — хорошее решение", "43:06: бой 3×4, Radiant 0–3", "Три buyback закончились dieback"],
  },
];
...
export const AXIS_COPY: Record<StageKey, Record<AxisKey, string>> = {
  draft: {
    movement: "Заранее связать стартовую расстановку с линиями и первыми рунами.",
    vision: "Стартовые варды оцениваются после выбора перспективы игрока.",
    resources: "Сравнивать не только коров, но и доступ к безопасному фарму линий.",
    macro: "Dire проще превращает выигранную драку в здания; Radiant нужна раздельная карта и быстрые одиночные цели.",
    micro: "Не стоять компактно против Black Hole; сохранить одного героя вне инициации Enigma.",
  },
  laning: {
    movement: "Ротация на 7:01 выиграла 2–1, но освободила Shadow Fiend: локальная победа стоила центральной T1 на 7:41.",
    vision: "Показываем реальные точки установленных вардов; активность конкретного варда без replay не выдумываем.",
    resources: "На 10:00 счёт был 6–6, но Dire имел 190 против 92 LH и +4 705 золота. Убийства не компенсировали фарм.",
    macro: "Падение mid T1 на 7:41 открыло Dire маршруты между обеими половинами карты.",
    micro: "First Blood на CK в 2:45 и смерть WR от SF в 6:25 ускорили уже проигрываемые по крипам линии.",
  },
  mid: {
    movement: "После Blink Enigma 14:31 и SF 14:42 прямой вход без информации стал невыгоден: искать противоположную сторону карты.",
    vision: "Перед Tormentor нужна линия обзора на подходы, а не только варды внутри собственного леса.",
    resources: "При −13,7k на 26:00 любой равный по числу героев файт начинался с экономического минуса.",
    macro: "Обмен T1 снизу не компенсировал Tormentor; нужен был следующий связанный объект или безопасный фарм двух линий.",
    micro: "Окна 15:24 и 16:40 показали связку Black Hole / Eclipse / Requiem: без разведки этих кнопок драку не начинать.",
  },
  late: {
    movement: "После снятия Aegis на 42:30 нужен был полный отход до возрождения Juggernaut и Ogre, а не встречный бой тройкой.",
    vision: "Драка 43:06 проиграна не из-за отсутствия варда: Observer и Sentry Radiant были рядом. Причина — состав 3×4 и глобальный минус.",
    resources: "Перед 43:06 Dire имел +7,5k золота и +11k XP. Внутри окна Radiant уступил ещё 6 586 золота и 9 906 XP.",
    macro: "После 43:06 требовалась единая защита; поочерёдные возвраты и три dieback оставили хайграунд без полного состава.",
    micro: "43:06: Black Hole с BKB наказал сгруппированную тройку. Один герой должен оставаться вне зоны инициации Enigma.",
  },
};
