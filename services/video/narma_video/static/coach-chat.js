...
export function mountCoachChat(host,{api,jobId,reportHash,evidence=[],context={},identity,isCurrent=()=>true,onEvidence=()=>{}}){
  ...
  let live=true,busy=false,pending=null,timer=null;
  ...
  host.append(heading,description,log,prompts,form,status);
  function render(turns){
    ...
    for(const turn of turns){
      ...
      if(turn.state==='succeeded'&&turn.answer){
        ...
        const links=el('div',undefined,'evidence-links');for(const id of turn.answer.evidence_ids??[]){const item=eventMap.get(id);if(!item)continue;const button=el('button',`${time(item.time)} · К эпизоду`,'evidence-link');button.type='button';button.addEventListener('click',()=>onEvidence(id));links.append(button);}reply.append(links);
  ...
  async function load(){
    ...
    try{const data=await api(`/api/replays/${encodeURIComponent(jobId)}/chat`);if(!current())return;
    ...
    }catch(error){if(current()){status.textContent=`Не удалось обновить разговор. ${error.message}`;send.disabled=busy;}}
  ...
  form.addEventListener('submit',async event=>{
    ...
    if(!pending||pending.question!==question||pending.evidence_id!==evidenceId)pending={id:crypto.randomUUID(),report_sha256:reportHash,question,evidence_id:evidenceId};
    busy=true;send.disabled=true;input.disabled=true;episode.disabled=true;status.textContent='Тренер разбирает твой вопрос…';
    try{const data=await api(`/api/replays/${encodeURIComponent(jobId)}/chat`,'POST',pending);if(!current())return;
      if(data.turn?.state!=='running'){pending=null;input.value='';}busy=false;await load();
    }catch(error){if(current()){busy=false;send.disabled=false;input.disabled=false;episode.disabled=false;status.textContent=`${error.message} Нажми «Обновить разговор», чтобы проверить ответ. Повторная отправка неизменённого вопроса использует тот же запрос.`;}}
  });
  void load();
}
