import { renderProfileGuidance } from './player-profile.js';
...
export function createPlayerProgram({host,api,current,onNavigate,onOpen,onCheck}){
  ...
  async function load(){
    ...
    try{
      ...
      if(data.player_profile&&['not_started','partial'].includes(data.player_profile.state)){const banner=el('aside',null,'profile-banner');banner.append(el('p','Подстроим тренировку под твою цель, опыт и время.'),action(data.player_profile.state==='partial'?'Продолжить настройку':'Настроить тренера',()=>onNavigate('player-profile')));host.append(banner);}
      ...
      if(data.focus){
        ...
        if(e.personalization){card.append(renderProfileGuidance(e.personalization));if(data.guidance?.revision!==e.personalization.revision)card.append(el('p','Профиль изменён. Это задание сохраняет настройки на момент создания; новые задания и вопросы тренеру используют текущие ответы.','help'));}
        if(!e.personalization&&data.guidance)card.append(renderProfileGuidance(data.guidance));
        ...
        if(data.guidance)card.append(renderProfileGuidance(data.guidance,{preliminary:true}));
      ...
      const more=el('div',null,'learning-actions');more.append(action('Профиль и цели',()=>onNavigate('player-profile'),'quiet'),action('Все тренировки и проверки',()=>onNavigate('learning'),'quiet'),action('Обновить',load,'quiet'));host.append(more);
