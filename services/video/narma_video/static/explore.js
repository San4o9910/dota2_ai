...
async function learn() {
  ...
  let serial = 0;
  page.innerHTML = `<div class="page-heading"><div><p class="eyebrow">Библиотека обучения</p><h1>Разберись в игре. Закрепи в матче.</h1><p>Уроки с примерами, типичными ошибками и практикой для твоей позиции. Разбери решение, проверь себя и перенеси навык в следующий матч.</p></div><a href="/my-learning" class="text-link">Моя практика ↗</a></div><div class="toolbar"><div class="field"><label for="learn-position">Твоя позиция</label><select id="learn-position">${positionOptions(position)}</select></div><p class="learning-toolbar-note" id="learn-position-note">Выбери позицию, чтобы увидеть упражнения для своей линии и роли. Общие игровые решения доступны всем.</p></div><div id="learning-library"><p class="page-status" role="status">Загружаем упражнения…</p></div>`;
