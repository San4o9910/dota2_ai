...
import { mountNarmaSignature } from './brand-motion.js';
const page = document.querySelector('#page-content');
const paths = { '/example':'example', '/start':'start', '/': 'home', '/heroes': 'heroes', '/builds': 'builds', '/learn': 'learn', '/practice': 'practice', '/updates': 'updates' };
...
const titles = { example:'Пример разбора', start:'Первый разбор', home: 'Играй осознаннее', heroes: 'Герои Dota 2', builds: 'Сборки и игровые задачи', learn: 'Обучение', practice: 'Тренажёр решений', updates: 'В мире Dota' };
...
function home() {
  page.innerHTML = `<div class="home-lead vision-arrival"><section class="practice-feature" aria-labelledby="home-heading"><p class="eyebrow">Dota 2 · твой личный ИИ-тренер</p><h1 id="home-heading">Разбери матч.<br><em>Измени следующую игру.</em></h1><p class="muted">Найди момент, где всё решилось. Пойми свой выбор. Забери одно конкретное действие в следующий матч.</p><div class="button-row"><a class="button primary" href="/replays">Разобрать свой матч <span aria-hidden="true">↗</span></a><a class="button subtle" href="/practice">Начать тренировку <span aria-hidden="true">→</span></a></div><p class="feature-note"><span><i aria-hidden="true"></i>Твои эпизоды и таймкоды</span><span><i aria-hidden="true"></i>Вопросы ИИ-тренеру</span><span><i aria-hidden="true"></i>Практика без регистрации</span></p></section><div id="home-signature"></div></div>
    <section class="vision-path" aria-label="От матча к следующей игре"><a href="/replays"><span class="vision-path-number" aria-hidden="true">01 / РАЗБЕРИ</span><h2>Твой матч по существу <span aria-hidden="true">↗</span></h2><p>Главный вывод, ключевой эпизод и план на следующую игру.</p></a><a href="/coach"><span class="vision-path-number" aria-hidden="true">02 / ПОЙМИ</span><h2>Обсуди с ИИ-тренером <span aria-hidden="true">↗</span></h2><p>Задавай вопросы о решениях и сравнивай свои матчи.</p></a><a href="/training"><span class="vision-path-number" aria-hidden="true">03 / ПРИМЕНИ</span><h2>Проверь себя в игре <span aria-hidden="true">↗</span></h2><p>Выбери одну цель и вернись к ней после следующих матчей.</p></a></section>
    <section class="home-section" aria-labelledby="home-heroes-heading"><div class="section-heading"><div><h2 id="home-heroes-heading">Знай своего героя</h2><p>Найди героя и разберись в его задачах.</p></div><a class="text-link" href="/heroes">Все герои <span aria-hidden="true">→</span></a></div><div id="home-heroes" class="hero-strip"><p class="page-status" role="status">Загружаем героев…</p></div></section><div class="home-bottom"><section aria-labelledby="home-learning-heading"><div class="section-heading"><div><h2 id="home-learning-heading">От линии к пониманию игры</h2><p>Одна ступень. Одно действие. Проверка в своей игре.</p></div><a class="text-link" href="/learn">6 ступеней <span aria-hidden="true">→</span></a></div><div id="home-stages" class="stage-preview-list"><p class="page-status" role="status">Открываем упражнения…</p></div></section><aside class="home-news" id="home-news" aria-label="В мире Dota"><div class="section-heading"><h2 id="home-news-heading">В мире Dota</h2><a class="text-link" href="/updates">Все новости <span aria-hidden="true">→</span></a></div><p class="page-status" role="status">Загружаем новости Valve…</p></aside></div>`;
  mountNarmaSignature(document.querySelector('#home-signature'));
...
function example(){
  page.innerHTML=`<div class="page-heading"><p class="eyebrow">Учебный пример · без регистрации</p><h1>Из одного эпизода — в одну тренировку</h1><p>Это вымышленная ситуация для знакомства с форматом. Она не является разбором реального игрока и не доказывает точность ИИ.</p></div><section class="surface program-focus"><p class="eyebrow">18:40 · решение перед выходом на линию</p><h2>Перед выходом проверь, кого видно на карте</h2><h3>Что произошло</h3><p>В учебной ситуации игрок вышел за реку за пачкой, когда несколько противников пропали с карты.</p><h3>Почему стоит пересмотреть решение</h3><p>Неизвестное положение противников повышает неопределённость. Сам факт смерти ещё не доказывает, что выход был ошибкой.</p><h3>Альтернатива</h3><p>Перед выходом сделать паузу: посмотреть миникарту, ближайших союзников и путь отхода. Если данных недостаточно — выбрать более безопасный участок.</p><h3>Когда подходит</h3><p>Когда есть выбор маршрута и нет срочной командной задачи. Видимые противники, поддержка команды или необходимость защитить объект могут изменить решение.</p><h3>Одна тренировка</h3><p class="program-action">Перед каждым выходом за реку назови себе видимых противников и путь отхода.</p><h3>Проверка следующего матча</h3><p>Найди один такой выход в новом реплее. Запиши, что было известно до решения и что изменил бы. Сравнение числа смертей помогает выбрать эпизод, но не оценивает решение автоматически.</p><a class="button primary" href="/start">Попробовать на своём матче</a></section>`;
}
function start(){
  page.innerHTML=`<div class="page-heading"><p class="eyebrow">Первый разбор</p><h1>Подготовь запись своего матча</h1><p>Понадобятся полный файл .dem и твой ник в этой игре.</p></div><section class="surface program-focus"><ol><li>Открой историю матчей в Dota 2, выбери игру и скачай реплей, если он доступен.</li><li>Найди .dem в папке игры dota/replays. Если у тебя .bz2, распакуй архив.</li><li>Создай аккаунт, выбери свой ник, позицию и тип разбора. Первый матч закрепляет игрока по Steam ID.</li><li>Дождись завершения передачи файла. Дальше обработка идёт на сервере; к отчёту можно вернуться в истории.</li><li>Выбери из отчёта одно действие, проверь его в новой игре и сохрани личную оценку.</li></ol><p>Загрузки и ответы ИИ в пилоте ограничены. Если доступ временно исчерпан, это будет показано до продолжения. Письмо о готовности пока не отправляется.</p><a class="button primary" href="/replays">Загрузить свой матч</a> <a class="text-link" href="/example">Сначала пример</a></section>`;
}
...
async function learn() {
  ...
  let serial = 0;
  page.innerHTML = `<div class="page-heading"><div><p class="eyebrow">Библиотека обучения</p><h1>Разберись в игре. Закрепи в матче.</h1><p>Уроки с примерами, типичными ошибками и практикой для твоей позиции. Разбери решение, проверь себя и перенеси навык в следующий матч.</p></div><a href="/training" class="text-link">Моя тренировка ↗</a></div><div class="toolbar"><div class="field"><label for="learn-position">Твоя позиция</label><select id="learn-position">${positionOptions(position)}</select></div><p class="learning-toolbar-note" id="learn-position-note">Выбери позицию, чтобы увидеть упражнения для своей линии и роли. Общие игровые решения доступны всем.</p></div><div id="learning-library"><p class="page-status" role="status">Загружаем упражнения…</p></div>`;
...
async function practice() {
  ...
  page.innerHTML = `<div class="page-heading"><div><p class="eyebrow">Практика Dota 2</p><h1>Тренажёр решений</h1><p>Прочитай ситуацию, выбери действие и разбери причину. Выбери серию из 5, 10 или 15 заданий. Разбор покажет ход решения, альтернативы и что проверить в своём повторе.</p></div><a href="/learn" class="text-link">Библиотека упражнений →</a></div><section id="practice-root" aria-label="Тренировка игровых решений"><p class="page-status" role="status">Открываем тренажёр…</p></section>`;
...
try { await ({ home, heroes, builds, learn, updates, practice, example, start }[route])(); }
