...
from agent_task import MODEL, REVISION
...
def main():
    ...
    try:
        ...
        for mode in ("valid", "malformed", "provider_error", "deadline", "valid"):
            ...
            try:
                result = run_isolated(validate_request(dict(REQUEST)), deadline=15 if mode == "deadline" else 30)
                assert mode == "valid", f"{mode} unexpectedly succeeded"
                assert json.loads(result["final_response"]) == FINAL
            ...
            print(json.dumps({"event": "hermes_actual_runtime_smoke", "case": mode,
