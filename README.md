...
## Новый независимый модуль видео (PostgreSQL)

`services/video` содержит API и worker для собственного сервера: загрузка видео,
очередь PostgreSQL, FFmpeg-декодирование каждого кадра, Gemini image batches
с журналом покрытия, приватные результаты и таймкоды. На сайте добавлен `/videos`
и серверный мост. Без адреса сервиса и ключей обработка не включается.
См. [запуск, ограничения и переход на RDS](services/video/README.md).

`services/replay` — отдельный Clarity CLI без OpenDota; на реальном реплее
проверены события 81 варда и 22 башен. Это выбранные обработчики событий,
не полный тренерский отчёт и не реализация точного тумана войны.

Новые сервисы не используют OpenDota. Старый контур ниже ещё существует:
его замена и перенос аккаунтов/D1/авторизации на собственный сервер не завершены.
Новый видеомодуль использует PostgreSQL сразу; RDS подразумевается PostgreSQL.
