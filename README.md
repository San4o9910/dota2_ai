...
- **Golden demo матча `8963624400`**: четыре стадии, выбор одного из десяти
  героев, карта и события, золото/опыт, 15 окон драк и тренировочные задачи по
  пяти навыкам. Demo не выдаёт себя за обработку произвольного Match ID.
- **Гейтированный NARMA Scan**: первый `POST /api/scan` получает Match ID и
  возвращает десять обезличенных слотов; второй принимает выбранный slot и
  возвращает один детерминированный числовой момент. Это не AI-вывод и не
...
В репозитории хранится оригинальная растровая тактическая подложка:

```text
public/tactical-map-background.webp
```

Она служит условным фоном, не выдаётся за точную карту патча и не копирует
графику Valve. Поверх неё интерфейс рисует объекты, точки установок вардов и
места смертей. Инструкция по
необязательной локальной подложке из клиента находится в
